...
public class HybridSAMLSSOServiceProviderDAOImpl implements SAMLSSOServiceProviderDAO {
    ...
    private static final Log LOG = LogFactory.getLog(HybridSAMLSSOServiceProviderDAOImpl.class);
    ...
    public boolean updateServiceProvider(SAMLSSOServiceProviderDO serviceProviderDO, String currentIssuer, int tenantId)
        ...
        if (registrySAMLSSOServiceProviderDAOImpl.isServiceProviderExists(currentIssuer, tenantId)) {
            registrySAMLSSOServiceProviderDAOImpl.removeServiceProvider(currentIssuer, tenantId);
            jdbcSAMLSSOServiceProviderDAOImpl.addServiceProvider(serviceProviderDO, tenantId);
            if (LOG.isDebugEnabled()) {
                LOG.debug(
                        "Moved SAML SSO service provider with issuer " + currentIssuer + " from registry to database.");
            }
        }
        jdbcSAMLSSOServiceProviderDAOImpl.updateServiceProvider(serviceProviderDO, currentIssuer, tenantId);
