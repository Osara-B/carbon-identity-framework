...
public class CacheBackedSAMLSSOServiceProviderDAO extends JDBCSAMLSSOServiceProviderDAOImpl {
    ...
    private final SAMLSSOServiceProviderListCache samlSSOServiceProviderListCache = SAMLSSOServiceProviderListCache.getInstance();
    ...
    public SAMLSSOServiceProviderDO getServiceProvider(String issuer, int tenantId) throws IdentityException {
        ...
        SAMLSSOServiceProviderDO samlSSOServiceProviderDO = samlSSOServiceProviderCache.getValueFromCache(issuer, tenantId);
        if (samlSSOServiceProviderDO != null) {
            if (LOG.isDebugEnabled()) {
                LOG.debug(String.format("Cache hit in SAMLSSOServiceProviderCache for issuer: %s, tenant: %d", issuer, tenantId));
            }
            return samlSSOServiceProviderDO;
        }
        if (LOG.isDebugEnabled()) {
            LOG.debug(String.format("Cache miss in SAMLSSOServiceProviderCache for issuer: %s, tenant: %d", issuer, tenantId));
        }
    ...
    public SAMLSSOServiceProviderDO[] getServiceProviders(int tenantId) throws IdentityException {
        ...
        SAMLSSOServiceProviderDO[] samlSSOServiceProviderDOList = samlSSOServiceProviderListCache.getValueFromCache(SAML_SP_LIST_CACHE_KEY, tenantId);
        if (samlSSOServiceProviderDOList != null) {
            if (LOG.isDebugEnabled()) {
                LOG.debug(String.format("Cache hit in SAMLSSOServiceProviderListCache for SAML Service Providers for tenant: %d", tenantId));
            }
            return samlSSOServiceProviderDOList;
        }
        if (LOG.isDebugEnabled()) {
            LOG.debug(String.format("Cache miss in SAMLSSOServiceProviderListCache for SAML Service Providers for tenant: %d", tenantId));
        }
